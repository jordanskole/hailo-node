 ...
 */
export enum HailoGenAIActionID {
  LLM__CREATE = 0,
...
export const GENAI_HEADER_SIZE = 4;
 ...
 */
export function packPayload(actionId: number, protobufBytes: Uint8Array): Buffer {
  const buf = Buffer.alloc(GENAI_HEADER_SIZE + protobufBytes.length);
  buf.writeUInt32LE(actionId, 0);
  buf.set(protobufBytes, GENAI_HEADER_SIZE);
  return buf;
}
 ...
 */
export function unpackPayload(buf: Buffer): { actionId: number; body: Buffer } {
  const actionId = buf.readUInt32LE(0);
  const body = buf.subarray(GENAI_HEADER_SIZE);
 ...
 */
export function packLengthPrefix(size: number): Buffer {
  const buf = Buffer.alloc(LENGTH_PREFIX_SIZE);
 ...
 */
export function readLengthPrefix(buf: Buffer): number {
